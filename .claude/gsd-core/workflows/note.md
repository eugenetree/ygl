...
- **Global scope**: `C:/projects/ygl/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/projects/ygl/.claude/notes/`)
...
2. Glob `C:/projects/ygl/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/projects/ygl/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/projects/ygl/.claude/notes/` — works in any directory
