...
export async function up(db: Kysely<any>): Promise<void> {
  await sql`CREATE TYPE video_job_skip_cause AS ENUM ('MEMBERS_ONLY', 'GEO_RESTRICTED')`.execute(
    db,
  );
  ...
  await sql`UPDATE "video_jobs" SET "skip_cause" = 'MEMBERS_ONLY' WHERE "status" = 'MEMBERS_ONLY'`.execute(
    db,
  );

  await sql`CREATE TYPE video_job_status_new AS ENUM ('PENDING', 'PROCESSING', 'SUCCEEDED', 'FAILED', 'SKIPPED')`.execute(
    db,
  );
  await sql`
  ...
  await sql`ALTER TYPE video_job_status_new RENAME TO video_job_status`.execute(
    db,
  );
}
...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`CREATE TYPE video_job_status_old AS ENUM ('PENDING', 'PROCESSING', 'SUCCEEDED', 'FAILED', 'MEMBERS_ONLY')`.execute(
    db,
  );
  await sql`
    ALTER TABLE "video_jobs"
  ...
  `.execute(db);
  await sql`DROP TYPE video_job_status`.execute(db);
  await sql`ALTER TYPE video_job_status_old RENAME TO video_job_status`.execute(
    db,
  );
