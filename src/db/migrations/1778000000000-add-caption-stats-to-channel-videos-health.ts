...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .addColumn("totalProcessedCount", "integer", (col) =>
      col.defaultTo(0).notNull(),
    )
    .addColumn("validCaptionsCount", "integer", (col) =>
      col.defaultTo(0).notNull(),
    )
...
export async function down(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .addColumn("succeededVideosStreak", "integer", (col) =>
      col.defaultTo(0).notNull(),
    )
    .addColumn("failedVideosStreak", "integer", (col) =>
      col.defaultTo(0).notNull(),
    )
    .addColumn("createdAt", "timestamp", (col) =>
      col.defaultTo(sql`now()`).notNull(),
