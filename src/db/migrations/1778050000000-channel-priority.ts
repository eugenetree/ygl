...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn("components", "jsonb", (col) =>
  ...
  await sql`CREATE INDEX ON "channel_jobs" (id) WHERE priority > 0 AND status = 'PENDING'`.execute(
    db,
  );
  await sql`CREATE INDEX ON "video_discovery_jobs" (id) WHERE priority > 0 AND status = 'PENDING'`.execute(
    db,
  );
  await sql`CREATE INDEX ON "video_jobs" (id) WHERE priority > 0 AND status = 'PENDING'`.execute(
    db,
  );
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .alterTable("channelEntries")
    .alterColumn("queryId", (col) => col.setNotNull())
    .execute();
