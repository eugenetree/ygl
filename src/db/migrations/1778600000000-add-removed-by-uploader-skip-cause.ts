...
export async function up(db: Kysely<any>): Promise<void> {
  await sql`ALTER TYPE video_job_skip_cause ADD VALUE 'REMOVED_BY_UPLOADER'`.execute(
    db,
  );
}
...
export async function down(db: Kysely<any>): Promise<void> {
  await sql`
    CREATE TYPE video_job_skip_cause_new AS ENUM ('MEMBERS_ONLY', 'GEO_RESTRICTED', 'AGE_RESTRICTED', 'PREMIERE')
  `.execute(db);
  ...
  await sql`
    ...
    USING (
      CASE WHEN "skip_cause"::text = 'REMOVED_BY_UPLOADER' THEN NULL
