...
describe("YoutubeApiGetVideo.getVideo()", () => {
  ...
  it("returns both tracks FETCHED with correct video metadata and captions for Wnp_cMw_GxM", {
    timeout: TIMEOUT,
  }, async () => {
    ...
    assert.ok(
      result.ok,
      `Expected success but got failure: ${JSON.stringify(!result.ok && result.error)}`,
    );
    ...
    assert.equal(
      video.title,
      "This Week at Lincoln Center\u2014June 15, 2017\u2014WNET",
    );
    ...
    assert.ok(
      video.keywords.includes("Lincoln Center"),
      `Expected keywords to include "Lincoln Center", got: ${video.keywords}`,
    );
    ...
    assert.ok(
      video.autoCaptions.data.length > 0,
      "Expected non-empty autoCaptions",
    );
    ...
    assert.equal(video.manualCaptions.state, "FETCHED");
    assert.ok(
      video.manualCaptions.data.length > 0,
      "Expected non-empty manualCaptions",
    );
  ...
  it("returns auto PRESENT_NOT_FETCHED + manual ABSENT for video with only auto captions (3IT2Cc_5WxE)", {
    timeout: TIMEOUT,
  }, async () => {
  ...
  it("returns auto ABSENT + manual PRESENT_NOT_FETCHED for video with only manual captions (SFnMTHhKdkw)", {
    timeout: TIMEOUT,
  }, async () => {
  ...
  it("detects Thai (th) via th-orig key despite yt-dlp reporting en-US (wda-B3NJSek)", {
    timeout: TIMEOUT,
  }, async () => {
  ...
  it("returns both tracks ABSENT for video with no captions (2aHCAfw95dI)", {
    timeout: TIMEOUT,
  }, async () => {
